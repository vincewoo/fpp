...
struct T final : public Fw::Serializable { // Extend Fw::Serializable
  ...
  SS serializeTo(B& b) const final { return b.serializeTo(x); }

  // Define the virtual deserialize method
  SS deserializeFrom(B& b) final { return b.deserializeFrom(x); }
...
Instead of implementing the `serializeFrom` and `deserializeTo` functions
...
uses these functions to implement `serializeFrom` and `deserializeTo`.
